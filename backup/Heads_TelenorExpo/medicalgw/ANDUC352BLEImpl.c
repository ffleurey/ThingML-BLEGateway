...
// BEGIN: Code from the c_global annotation ANDUC352BLEImpl
  ...
  static ble_random_number_t peer_ltk = { 0xAA, 0x9F, 0xF1, 0xBE, 0x40, 0x04, 0x77, 0xB3, 0x68, 0x64, 0x9D, 0x2D, 0x6E, 0x69, 0xBD, 0xAD, };
  static uint16_t peer_ediv = 0x04F1;
  static ble_random_part_t peer_random = { 0x33, 0x40, 0xCC, 0x17, 0xAF, 0x96, 0x41, 0xFE, };
...
//End Region Notify
ANDUC352BLEImpl_States_State_event_consumed = 0 | ANDUC352BLEImpl_States_Notify_State_event_consumed ;
//End dsregion Notify
if (ANDUC352BLEImpl_States_State_event_consumed == 0 && 1) {
...
//End Region Sync
ANDUC352BLEImpl_States_State_event_consumed = 0 | ANDUC352BLEImpl_States_Sync_State_event_consumed ;
//End dsregion Sync
if (ANDUC352BLEImpl_States_State_event_consumed == 0 && 1) {
...
void enqueue_ANDUC352BLEImpl_Tellu_MqttRequestWeightMeasurement(struct ANDUC352BLEImpl_Instance * inst) {
    fifo_lock(&(inst->fifo));
    if ( fifo_byte_available(&(inst->fifo)) > 4 ) {
...
void enqueue_ANDUC352BLEImpl_Tellu_MqttConfirmWeightMeasurement(struct ANDUC352BLEImpl_Instance * inst) {
    fifo_lock(&(inst->fifo));
    if ( fifo_byte_available(&(inst->fifo)) > 4 ) {
...
switch(portID) {
case 1:{
ANDUC352BLEImpl_handle_Tellu_MqttRequestWeightMeasurement(_instance);
return 1;
break;
}
}
...
switch(portID) {
case 1:{
ANDUC352BLEImpl_handle_Tellu_MqttConfirmWeightMeasurement(_instance);
return 1;
break;
}
}
